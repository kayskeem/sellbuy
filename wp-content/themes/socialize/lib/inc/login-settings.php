...
if ( isset( $_POST['action'] ) && $_POST['action'] == 'ghostpool_login' ) {
	...
	if ( $captcha && $captcha['reason'] != '' ) {
		echo "<span class='error'>" . esc_html__( 'Incorrect captcha.', 'socialize' ) . "</span>";	
		exit();
...
if ( isset( $_POST['action'] ) && $_POST['action'] == 'ghostpool_register' ) {
	...
	if ( is_wp_error( $user_register ) ) {	
		...
		if ( in_array( 'empty_user_login', $error ) ) {
			echo "<span class='error'>" . $user_register->get_error_message( 'empty_user_login' ) . "</span>";	
			exit();
...
if ( isset( $_POST['action'] ) && $_POST['action'] == 'ghostpool_lost_password' ) {
	...
	if ( $message && ! wp_mail( $user_email, esc_html__( 'Password reset request', 'socialize' ), $message ) ) {
...
if ( isset( $_GET['key'] ) && isset( $_GET['action'] ) && $_GET['action'] == 'reset_pwd' ) {
	...
	if ( ! empty( $reset_key ) && ! empty( $user_data ) ) {
		...
		if ( $message && ! wp_mail( $user_email, esc_html__( 'Your new password', 'socialize' ), $message ) ) {
			echo "<span class='error'>" . esc_html__( 'Email failed to send for some unknown reason', 'socialize' ) . "</span>";
			exit();
		} else {
